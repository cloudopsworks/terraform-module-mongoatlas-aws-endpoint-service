...
dependency "endpoint" {
  ...
  mock_outputs_allowed_terraform_commands = ["validate", "destroy"]
  mock_outputs = {
    endpoint_privatelink_id           = "123456687595769785967"
    endpoint_privatelink_service_name = "com.amazonaws.vpce.us-east-1.vpce-svc-7289475uy3f5f53"
...
inputs = {
  ...
  {{- if ne .Name "org" }}
  {{- if and $.endpoint (eq .Name "private_link_id") }}
  {{ .Name }} = dependency.endpoint.outputs.endpoint_privatelink_id
  {{- else }}
